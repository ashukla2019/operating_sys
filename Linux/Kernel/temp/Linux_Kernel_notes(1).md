...
Return path:
Hardware -> Driver -> Kernel subsystem -> Syscall return
...
```text
CPU │ +---------------+ | User Space    | | Applications  | +---------------+ System Calls │ +---------------+ | Kernel Space  | | Linux Kernel  | +---------------+ │ Hardware Devices 
```
...
```text
Applications cannot directly perform privileged operations — instead they request the kernel. This request is called a **System Call** . Example: <mark>printf() → write() → System Call → Kernel → Terminal</mark> 
```
...
```text
Flow: <mark>Application → glibc → write() → System Call → Kernel → Terminal Driver → Screen</mark> 
```
...
```text
Example: <mark>printf() → glibc → write() → Kernel</mark> 
```
...
```text
**Kernel Module Flow:** <mark>Driver.ko → insmod → Kernel → Driver Initialized → Device Ready</mark> 
```

## **Linux Boot Process (High Level)** 

```text
Power ON → BIOS/UEFI → Bootloader (GRUB) → Linux Kernel → Initramfs → systemd (PID 1) → Services → Login → Applications 
```
...
```text
Flow: <mark>cat → glibc → open() → Kernel → VFS → ext4 → Block Layer → Storage Driver → SSD → Data Returned → cat prints file</mark> Every file access passes through the kernel. 
```

**Another example** — Typing: <mark>ping google.com</mark> 

```text
Flow: <mark>ping → socket() → Kernel Network Stack → NIC Driver → Network Card → Internet → Reply → Kernel → Application</mark> Applications never communicate with hardware directly. 
```
...
Both processes access the same mapped memory.
...
# **Simple Shared Memory Example in C (POSIX)** This example demonstrates how to use **POSIX Shared Memory** with **`** shm_open() **` and `** mmap() **`** . --# **# Writer Program (`writer.c`) ```** c #include **<stdio.h>** #include **<fcntl.h>** #include **<sys/mman.h>** #include **<unistd.h>** #include **<string.h>** int main() { const char *name = "/my_shared_memory"; const int SIZE = 1024; _// Create shared memory object_ int shm_fd = shm_open(name, O_CREAT | O_RDWR, 0666); _// Set the size_ ftruncate(shm_fd, SIZE); _// Map shared memory_ char *ptr = mmap(NULL, SIZE, PROT_WRITE, MAP_SHARED, shm_fd, 0); _// Write data_ strcpy(ptr, "Hello from shared memory!"); printf("Data written: %s\n", ptr); munmap(ptr, SIZE); close(shm_fd); **return** 0; } 

## **Reader Program** **<mark>( reader.c</mark> )** 

#include **<stdio.h>** #include **<fcntl.h>** #include **<sys/mman.h>** #include **<unistd.h>** int main() { const char *name = "/my_shared_memory"; const int SIZE = 1024; 

_// Open existing shared memory_ int shm_fd = shm_open(name, O_RDONLY, 0666); _// Map shared memory_ char *ptr = mmap(NULL, SIZE, PROT_READ, MAP_SHARED, shm_fd, 0); _// Read data_ printf("Data read: %s\n", ptr); munmap(ptr, SIZE); close(shm_fd); _// Delete shared memory object (optional)_ shm_unlink(name); **return** 0; } 

## **Compile** 

gcc writer.c -o writer -lrt gcc reader.c -o reader -lrt 

**Note:** On many modern Linux systems, <mark>-lrt</mark> is not required. 
...
|close()|Closes the shared memory fle descriptor.|
...
Since both processes access the same memory, synchronization is required. Common tools: Mutex, Semaphore, Reader-Writer Lock, Condition Variable, Spinlock. Otherwise, race conditions occur.

The writer creates the shared memory, writes a message into it, and exits. The reader opens the same shared memory object, reads the message, and optionally removes the shared memory using <mark>shm</mark> _ <mark>unlink() .</mark> 

--## Advantages - Fastest IPC - No data copying - Excellent for large data --## Disadvantages - Requires synchronization - Complex programming - Local machine only --## Use Cases - Multimedia applications - Video streaming - Sensor data - Database shared cache --## Don't Use When 

- Processes are on different machines - Synchronization cannot be guaranteed 

--- 

# 7. Message Queue ## Concept 

A Message Queue stores structured messages inside the kernel. Processes communicate by sending and receiving messages. Communication is asynchronous. 
...
## System Calls ```cpp msgget() msgsnd() msgrcv() msgctl() 
...
**```** c #include **<stdio.h>** #include **<fcntl.h>** #include **<sys/stat.h>** #include **<mqueue.h>** #include **<string.h>** int main() { mqd_t mq; **struct** mq_attr attr; attr.mq_flags = 0; attr.mq_maxmsg = 10; attr.mq_msgsize = 100; attr.mq_curmsgs = 0; 

_// Create/Open message queue_ mq = mq_open("/myqueue", O_CREAT | O_WRONLY, 0666, &attr); 

char msg[] = "Hello from Sender!"; 

_// Send message_ mq_send(mq, msg, strlen(msg) + 1, 0); 

printf("Message Sent: %s\n", msg); mq_close(mq); 

**return** 0; } 

## **Receiver Program (** **<mark>receiver.c</mark> )** 

#include **<stdio.h>** #include **<fcntl.h>** #include **<mqueue.h>** 

int main() { mqd_t mq; char buffer[100]; 

_// Open existing message queue_ mq = mq_open("/myqueue", O_RDONLY); 
...
gcc sender.c -o sender -lrt gcc receiver.c -o receiver -lrt 

**Note:** On many modern Linux systems, <mark>-lrt</mark> may not be required. 
...
# 8. Socket ## Concept Sockets provide bidirectional communication between processes. They can communicate: 

- On the same machine - Across different machines Sockets form the basis of client-server programming. --## Types 

### Unix Domain Socket 

Local machine communication. ### TCP Socket Reliable network communication. ### UDP Socket Fast but unreliable communication. --## How It Works ```text
Client Process
     |
     | socket()/connect()
     v
...
--## Common System Calls ```cpp socket() bind() listen() accept() connect() send() recv() close() 
...
With MAP_SHARED, both processes can see shared modifications.
```
...
```text
Process creation -> runnable execution (Linux mental model)
...
Create task_struct
      |
      v
Create/prepare address-space state
(mm_struct may be shared or separate)
      |
      v
Set up kernel stack + scheduling state
      |
      v
...
```text
P1 holds Lock A, waiting on Lock B
P2 holds Lock B, waiting on Lock A
```
...
(Copy-on-Write, pages initially read-only)
  |
  | one process writes
  v
Page fault
...
Child is re-parented
      |
      v
PID 1 (systemd/init) becomes parent
``` 
...
Syscall Dispatcher
     ...
     | syscall number
     v
sys_write() / appropriate handler
     |
     v
Kernel subsystem
     |
     | if device I/O is needed
     v
Device Driver
     |
     v
...
Running Program
      ...
      | interrupt arrives
      v
CPU saves required execution state
      |
      v
Enter kernel / interrupt context
      |
      v
Interrupt handler / ISR
      |
      v
Acknowledge + handle event
      |
      v
...
Resume interrupted task
      |
      +--> (or schedule another runnable task)
``` 
...
Generated by the system clock. Used for: CPU scheduling Multitasking Time sharing Example: ```text
Process A running
       |
       | timer IRQ
       v
...
Keyboard ISR / Driver
      |
      v
Store event / character
      |
      v
Wake waiting task if needed
      |
      v
...
```text
CPU ↓ Needs Page ↓ Page Present? ↓ Yes 
```

↓ 

```text
Execute ↓ No ↓ Page Fault ↓ Load from Disk ↓ Continue 
```
...
```text
A driver registers an interrupt handler, e.g. <mark>request</mark> _ <mark>irq(irq, handler, ...); .</mark> The handler is invoked when the corresponding interrupt occurs: <mark>Device --> IRQ --> handler()</mark> . 
```
...
```text
Critical distinction. **Process Context** — <mark>System call --> Driver ;</mark> the driver executes on behalf of a process and can generally sleep, block, use mutexes, and perform blocking operations. **Interrupt Context** — <mark>Hardware --> IRQ Handler</mark> ; the handler cannot sleep, must execute quickly, and uses atomic/IRQ-safe synchronization. 
```
...
```text
The bottom half handles work that doesn’t need to happen immediately: <mark>Hardware --> Top Half --> Bottom Half --> Longer Processing .</mark> Linux provides several mechanisms for deferred work. 
```
...
```text
Softirqs are a mechanism for deferred kernel work — they run in an atomic context and therefore cannot sleep: <mark>Hard IRQ --> Softirq --> Deferred processing .</mark> Used by important kernel subsystems, including networking. 
```

## **18. Tasklets** 

```text
Tasklets were historically used for deferred interrupt processing: <mark>Hard IRQ --> Tasklet --> Deferred work</mark> . Important interview point: tasklets cannot sleep. For modern driver development, workqueues and threaded IRQs are often more relevant. 
```

## **19. Workqueue** 

```text
A workqueue executes deferred work in process context: <mark>IRQ --> Schedule Work --> Workqueue --> Worker Thread --> Driver Processing .</mark> Because it runs in process context, the work can generally sleep when appropriate. 
```

## **20. Threaded IRQ** 

```text
Linux supports threaded interrupt handlers: <mark>Hardware IRQ --> Primary IRQ Handler --> IRQ Thread --> Longer Processing .</mark> Useful when interrupt processing requires operations that can sleep. 
```
...
```text
The important architecture: <mark>IRQ --> Read status, Acknowledge, Schedule work --> Return quickly .</mark> 
```
...
```text
PCI/PCIe devices can use **Message Signaled Interrupts** — instead of relying on a traditional physical interrupt line, the device generates an interrupt through a memory transaction mechanism: <mark>PCIe Device --MSI--> Interrupt System --> CPU</mark> . 
```
...
```text
Linux networking uses NAPI to reduce interrupt overhead under high packet rates. Low traffic: <mark>Interrupt --> Process packets .</mark> High traffic: <mark>Interrupt --> Disable/reduce further RX interrupts --> Polling --> Process batch of packets --> Re-enable interrupts .</mark> The goal is to combine interrupt-driven notification with polling for efficient packet processing. 
```

## **40. Why NAPI?** 

```text
Under extremely high traffic, per-packet interrupts become excessive overhead. NAPI allows <mark>IRQ --> Poll many packets --> Batch processing ,</mark> improving throughput and reducing interrupt overhead. 
```
...
```text
An application waiting for device data: <mark>Application --> read() --> Wait Queue --> Sleep</mark> . Device receives data: <mark>Hardware --> IRQ --> Driver --> Wake Up --> Application</mark> . The application becomes runnable again. 
```

## **44. Interrupt + Completion** 

```text
Another common pattern: <mark>Process Context --> Start hardware operation --> wait</mark> _ <mark>for</mark> _ <mark>completion() --> Sleep</mark> . Hardware finishes: <mark>Hardware --> IRQ --> Driver --> complete() --> Wake process</mark> . A clean synchronization model for device operations. 
```
...
```text
1. **Interrupt not cleared** — <mark>Device --> IRQ --> Handler</mark> fails to clear it, so the interrupt remains active and fires again and again → interrupt storm. 
```
...
```text
4. **Excessive handler work** — <mark>IRQ --> Huge processing --> High latency ;</mark> move expensive work to an appropriate deferred context. 
```
...
```text
       ...
       MMIO               DMA                IRQ
        │                  │                  │
    Configure       Interrupt Handler   Completion
    Hardware               │
                            ▼
                     Wake/Notify
                            │
...
SOCK_STREAM    ─────► TCP
SOCK_DGRAM     ─────► UDP
SOCK_RAW       ─────► Raw IP/packet access
SOCK_SEQPACKET ─────► Sequenced reliable messages
```
...
|---|
```text
||<br>v<br>send()<br>|<br>v|
|Socket Layer<br>|<br>v|
|TCP<br>|<br>v<br>IP<br>|<br>v|
|Routing<br>|<br>v|
|Netdevice|
||<br>v|
|NIC Driver<br>|<br>v|
...
```text
Packet 1 → IRQ Packet 2 → IRQ Packet 3 → IRQ ... 
```
...
|Conceptually:|
```text
|Application<br>|<br>v<br>Network Stack<br>|<br>v<br>qdisc / traffic control<br>|<br>v|
```
...
```text
NIC | +-- RX Queue 0 → CPU 0 +-- RX Queue 1 → CPU 1 +-- RX Queue 2 → CPU 2 +-- RX Queue 3 → CPU 3 This is important for multicore networking performance. 
```
...
```text
epoll | +--------+--------+ |        |        | v        v        v Sock A   Sock B   Sock C 
```
...
```text
epoll | +--------+--------+ |        |        | v        v        v Client A  Client B  Client C | v Event Loop | v Process Ready Events 
```
...
```text
recv() | | no data v Task sleeps Nonblocking: recv() | | no data v Return immediately Nonblocking sockets are commonly combined with: epoll 
```
...
|---|
```text
||<br>v<br>NIC<br>|<br>v<br>Interrupt<br>|<br>v<br>NAPI<br>|<br>v<br>Packet processing<br>|<br>v<br>Socket buffer<br>|<br>v<br>Wake sleeping task<br>|<br>v<br>Scheduler<br>|<br>v<br>Application|
|Therefore:|
|Networking<br>+<br>Interrupts<br>+<br>NAPI<br>+<br>Memory/DMA<br>+<br>Scheduler<br>+<br>Sockets|
```
...
```text
skb ↓ packet data ↓ network headers ↓ transport headers ↓ metadata 
```
...
```text
NIC | +-- RX0 → CPU0 +-- RX1 → CPU1 +-- RX2 → CPU2 +-- RX3 → CPU3 
```
...
|---|
```text
|<br>+---------+---------+<br>|                   |<br>send()              recv()<br>|                   ^<br>v                   ||
```

```text
  Socket Layer   TCP / UDP  →  IP Layer   Routing / Netfilter  →  Network Device   +-----+-----+    TX Queue     RX Queue  ^  →  NIC Driver     +---- DMA --+  →  NIC ------------------+
```
...
```text
VFS | v Filesystem | v Block Layer /    |    \ /     |     \ SATA   NVMe   USB |      |      | SSD    SSD    Disk 
```
...
|Conceptually:|
```text
|Application<br>|<br>v<br>Direct I/O<br>|<br>v<br>Filesystem<br>|<br>v<br>Block Layer<br>|<br>v<br>Driver<br>|<br>v<br>Storage|
```
...
```text
Block Device | v Request Queue /     |      \ /      |       \ READ    WRITE    READ 
```
...
```text
Process context     → can sleep Interrupt context    → CANNOT sleep Softirq/Tasklet       → CANNOT sleep 
```
...
```text
CPU0 → my_counter (private copy) CPU1 → my_counter (private copy) CPU2 → my_counter (private copy) 
```
...
```text
Short, can't sleep, maybe interrupt context   → Spinlock (irqsave) Might sleep / longer critical section          → Mutex Counting resource, N holders                   → Semaphore Simple counter                                 → Atomic ops Per-CPU stats                                  → Per-CPU variables Read-mostly small scalar data                  → Seqlock Read-hot pointer/list/tree, rare writes         → RCU 
```
...
```text
|Core A reads X       → E (exclusive, only copy)<br>Core B also reads X  → both go to S (shared)<br>Core A writes X      → A goes to M, B's copy is invalidated → I<br>Core B reads X again → must fetch fresh copy from Core A/memory → back to S|
```
...
```text
EL0-EL3          → ARM's privilege model; EL2 for virtualization, EL3 for TrustZone/firmware Device Tree        → replaces hardcoded board files; static hardware description parsed at boot MESI/MOESI          → hardware cache coherency; explains barriers, false sharing, per-CPU variable value cpufreq              → how fast a core runs cpuidle               → what a core does when idle Runtime PM             → per-device power management, independent of CPU state PCIe vs Device Tree      → self-describing external devices vs static internal peripheral description 
```
...
```text
WARN_ON        → logged, system continues Oops            → task killed, system usually survives Panic            → system halted; kdump captures a vmcore for postmortem RIP + Call Trace   → where and how you got there addr2line            → RIP offset → exact source line crash + vmcore        → full postmortem state, bt -a for cross-CPU races ftrace                 → live function-level tracing, latency tracers (irqsoff/preemptoff) perf                    → statistical profiling, hardware counters, cache/IPC analysis KASAN/KFENCE              → memory-safety bugs (UAF, OOB) KCSAN                       → data races lockdep                      → potential deadlocks from lock ordering 
```
...
```text
**Mapping back to earlier chapters:** - <mark>mm</mark> → Chapter 5 (memory management) — this is the pointer to the process’s <mark>mm</mark> _ <mark>struct</mark> holding its page tables and VMAs. - <mark>se</mark> (a <mark>struct sched</mark> _ <mark>entity</mark> ) → Chapter 2 — this is where <mark>vruntime</mark> actually lives; the CFS redblack tree is built from these embedded structs, not from <mark>task</mark> _ <mark>struct</mark> pointers directly (see below). - <mark>cpus</mark> _ <mark>allowed</mark> → Chapter 2 §CPU Affinity. - <mark>files / fs</mark> → Chapter 3 (VFS) — the open file descriptor table <mark>( struct file *</mark> array) hangs off here. 
```
...
```text
platform_driver     → probe()/remove(), device-tree matched (Chapter 10) ├── devm_ioremap_resource   → MMIO region from "reg" ├── platform_get_irq         → IRQ from "interrupts" ├── devm_clk_get              → clock from "clocks" ├── devm_request_irq           → top half (Chapter 6) 
```
...
```text
task_struct     → se (sched_entity/vruntime, Ch.2), mm (Ch.5), files (Ch.3) pick_next_task    → red-black tree, leftmost = smallest vruntime wait_event/wake_up  → sleep/wake pattern, ALWAYS re-check condition in a loop 
```
...
```text
#include **<fcntl.h>** #include **<unistd.h>** #include **<stdio.h>** #include **<stdlib.h>** int main(void) { int fd = open("scratch.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644); **if** (fd == -1) { perror("open"); exit(EXIT_FAILURE); } const char *msg = "hello, system programming\n"; ssize_t written = 0, len = (ssize_t)strlen(msg); _/* write() can do a "short write" -- loop until everything is sent */_ **while** (written < len) { ssize_t ret = write(fd, msg + written, len - written); **if** (ret == -1) { perror("write"); close(fd); exit(EXIT_FAILURE); } written += ret; } close(fd); fd = open("scratch.txt", O_RDONLY); **if** (fd == -1) { perror("open"); exit(EXIT_FAILURE); } char buf[64]; ssize_t n; **while** ((n = read(fd, buf, **sizeof** (buf))) > 0) write(STDOUT_FILENO, buf, n); _/* echo it back to stdout */_ **if** (n == -1) perror("read"); close(fd); **return** 0; } 
```
...
```text
_/* writer.c */_ #include **<stdio.h>** #include **<stdlib.h>** #include **<fcntl.h>** #include **<unistd.h>** #include **<sys/mman.h>** #include **<string.h>** #define SHM_NAME "/my_shared_mem" #define SHM_SIZE 4096 int main(void) { int fd = shm_open(SHM_NAME, O_CREAT | O_RDWR, 0666); **if** (fd == -1) { perror("shm_open"); exit(EXIT_FAILURE); } ftruncate(fd, SHM_SIZE); char *addr = mmap(NULL, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0); **if** (addr == MAP_FAILED) { perror("mmap"); exit(EXIT_FAILURE); } strcpy(addr, "hello from writer process"); printf("writer: wrote message\n"); munmap(addr, SHM_SIZE); close(fd); **return** 0; _/* segment persists until shm_unlink() -- run reader next, then unlink */_ } _/* reader.c */_ #include **<stdio.h>** #include **<stdlib.h>** #include **<fcntl.h>** #include **<unistd.h>** #include **<sys/mman.h>** #define SHM_NAME "/my_shared_mem" #define SHM_SIZE 4096 int main(void) { int fd = shm_open(SHM_NAME, O_RDONLY, 0666); **if** (fd == -1) { perror("shm_open"); exit(EXIT_FAILURE); } char *addr = mmap(NULL, SHM_SIZE, PROT_READ, MAP_SHARED, fd, 0); **if** (addr == MAP_FAILED) { perror("mmap"); exit(EXIT_FAILURE); } printf("reader: %s\n", addr); munmap(addr, SHM_SIZE); close(fd); 
```
...
High priority H
    ...
    BLOCKED
      ...
      | lock held by
      |
Low priority L  <---- holds L
      ^
      |
Medium priority M keeps running
```
...
Hard IRQ handler
  |
  +--> acknowledge / capture state
  +--> minimal work
  +--> schedule deferred work
  |
  v
Thread/workqueue/tasklet/other deferred context
  |
  v
Longer processing
```
...
Need to protect shared data?
          ...
          |        +--> YES -> mutex
          |        |
          |        +--> NO -> spinlock / appropriate atomic primitive
          |
          +--> Read-mostly and lifetime-friendly?
                   |
                   +--> Consider RCU
```
...
Identify context
  ...
  |
  v
Check resource ownership/lifetime
  |
  v
...
Who can sleep?
Who can interrupt?
Who performs the DMA?
What address space is involved?
What synchronizes producer and consumer?

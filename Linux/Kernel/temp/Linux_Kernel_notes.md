...
- This example demonstrates how to use **POSIX
- Shared
- Memory** with `shm_open()` and `mmap()`.
...
**What is IPC?**
Why is IPC needed? Name different IPC mechanisms.
...
How does a message queue work? What is a Unix domain socket? What is the difference between TCP and Unix sockets? Explain mmap() . ``` Need synchronization?
...
- spin_lock_irqsave()
- Yes
- Yes context and you don't know the
...
- Poor (readers contend on the lock's cache line)
- Excellent
...
- Device
- Tree
- Blob — compiled binary form the bootloader hands to the
 .dtb
                                                                         kernel
```
...
- PCIe
- Switch ──►
- NVMe / WiFi / discrete devices (self-describing via PCI config space)
```
...
- KCSAN (Kernel
- Concurrency
- Sanitizer) memory accesses to detect racing reads/writes
                                                to the same memory
